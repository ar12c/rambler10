# RamblerPort v1.6 - full installer + verifier
...
#      (module = yogi fingerprint system.prop + system/product/... sysconfig XML,
#      the layout mountify actually mounts into /product/etc/sysconfig, plus the
#      Reborn runtime-overrides db for Gboard which service.sh self-heals)
#   2. Re-enables ramblerport and zygisk_vector (Vector/LSPosed must stay ON)
#   3. Repairs the mangled pif.json with coherent stock rango values
#      (Rambler comes from the Reborn runtime override hook inside Gboard, so GMS
#      can keep the stock identity and integrity stays consistent w/ TrickyStore)
#   4. Adds the Play Integrity checker to TrickyStore's target.txt
#   5. Reboots, waits for boot, verifies: yogi props, XML mount, 2026 feature
#   6. Smoke-tests Pixel Camera (fingerprint-only spoof is pairip-safe), then
#      restarts Gboard so it re-reads feature flags
#   7. Launches Play Integrity API Checker, taps "Check", scrapes the verdict
#
# After install: verify with verify_rambler.sh (hook log must show
# "override applied ... enable_agentic_dictation=true"). If the override store
# is ever wiped (Gboard data clear), re-run: make_overrides_db.py, push
# rambler_overrides.db + apply_overrides.sh to /data/local/tmp, run it as root.
...
Step '1. Normalize LF'
$textFiles = @('module.prop', 'action.sh', 'service.sh', 'system.prop',
...
# Binary payloads ride along in the module (never LF-normalized). The overrides
# db is regenerated by make_overrides_db.py - run it if rambler_overrides.db
# is missing or Gboard renamed its flags again.
$binFiles = @('rambler_overrides.db')
foreach ($rel in $binFiles) {
    if (-not (Test-Path (Join-Path $Root $rel))) { throw "$rel missing - run: python3 make_overrides_db.py" }
}
...
Write-Host "`nDone. If FEATURE-MISSING persists: mountify (auto mode) stages only modules having a system/ dir and no disable/remove/skip_mountify marker - check /data/adb/modules/ramblerport." -ForegroundColor Cyan
